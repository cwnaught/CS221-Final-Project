...
for key,val in dirDict.items():
    ...
    for file in os.listdir(key):
        ...
        if file in Test:
            if not os.path.isdir('data/color/Test'):
                os.mkdir('data/color/Test')
                os.mkdir('data/blackwhite/Test')
            np.save('data/color/Test/' + file[:-4] + '_cl',movie_c)
            np.save('data/blackwhite/Test/' + file[:-4] + '_bw',movie_bw)
        elif file in Valid:
            if not os.path.isdir('data/color/Valid'):
                os.mkdir('data/color/Valid')
                os.mkdir('data/blackwhite/Valid')
            np.save('data/color/Valid/' + file[:-4] + '_cl',movie_c)
            np.save('data/blackwhite/Valid/' + file[:-4] + '_bw',movie_bw)
        else:
